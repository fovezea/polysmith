...
- `create_body_copy { source_body_id }` adds a core-owned `body_copy` timeline feature. The compiler resolves the source body during replay, emits a new body under the copy feature id, preserves the source local frame, and marks the copy `dependency_broken` if the source body can no longer be resolved.
