...
- `add_sketch_arc { start_x, start_y, end_x, end_y, anchor_x, anchor_y, mode }` creates a sketch arc on the active sketch. `mode` is one of `three_point` (anchor lies on the arc; center = circumcenter of start, anchor, end) or `center_start_end` (anchor is the center; end is snapped onto the resulting circle). Endpoints participate in the shared sketch-point graph and are stored as fixed (v1 freezes arc shape at creation; reshape requires delete + redraw). The core rejects colinear / zero-radius input as a structured error. Arc edges contribute to closed-profile loop detection alongside lines, with interior points sampled into the profile so OCCT extrudes a clean curved boundary
