...
**Draft dimension Enter exits line tool.** Pressing Enter in the draft dimension input commits the line, then switches the sketch tool to "select" mode — matching how Fusion 360 and other CAD programs end the line command when dimensions are explicitly typed. Mouse click (without Enter) still commits and keeps the line tool active for chained drafting.

**FEATURE REQUEST: Double-click to break line chain.** Fusion 360 uses double-click at the last endpoint to end the chain but stay in line tool (unchained) — the next click starts a fresh independent line. PolySmith's pointer event system currently has no click-timing tracking for double-click detection during line drafting. Implementing this requires adding a timestamp/position tracker in the pointer-down handler and clearing `lineDraftStartRef` when two clicks land at the same location within ~300ms while a line draft session is active.
