...
- `create_thread`, `update_thread_parameters`, and `confirm_thread` add the semantic thread feature contract. This first pass stores cosmetic thread intent for a target body plus a sketch-line / construction-axis / straight-edge axis source; modeled helical booleans are staged behind the helix/sweep stack. Dependency refresh re-resolves both references and marks the feature `dependency_broken` when either target disappears.
- `create_fastener` and `update_fastener_parameters` add the semantic fastener feature contract with standard, size, length, thread length, head type, drive type, and thread representation parameters. The initial native shape is a simple shaft/head body while standards and real modeled thread geometry grow around the same feature kind.
