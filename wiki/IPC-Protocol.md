...
- `sweep_profile { profile_id, path_entity_id }` creates a new body by sweeping one closed sketch profile along a sketch line path. The profile and path may come from different sketches. v1 supports straight sketch-line paths; `update_sweep_profile { feature_id, profile_id }` and `update_sweep_path { feature_id, path_entity_id }` drive live preview and timeline editing. Sketch edits re-resolve the source profile plane and path line by id where possible; failures mark the sweep `dependency_broken` with a warning.
